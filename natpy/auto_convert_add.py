...
def convert_ufunc(self, function, method, *args, **kwargs):
    if function.__name__ != "add":
        ...
        unit = self.unit
        new_args = [x.convert(unit) if (
            isinstance(x, Quantity)) else x for x in args]
        new_kwargs = {key: (value.convert(unit) if isinstance(
            value, Quantity) else value) for key, value in kwargs.items()}
