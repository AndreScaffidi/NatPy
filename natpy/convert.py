...
def ConversionDimensionality(initial_unit, target_unit):
    ...
    initial_dimvec = [initial_unit.decompose().powers, initial_unit.decompose().bases]
    target_dimvec = [target_unit.decompose().powers, target_unit.decompose().bases]
    # Change-of-Unit Dimensionality Vector
    cou_dimvec = numpy.empty_like(natural_bases, dtype=float)
    for i in range(len(natural_bases)):
        cou_dimvec[i] = (
            target_dimvec[0][target_dimvec[1].index(natural_bases[i])]
            if natural_bases[i] in target_dimvec[1]
            else 0
        ) - (
            initial_dimvec[0][initial_dimvec[1].index(natural_bases[i])]
            if natural_bases[i] in initial_dimvec[1]
            else 0
        )
    return cou_dimvec
...
def UnitConvert(initial_unit, target_unit):
    ...
    scale_conv_factor = (
        initial_unit * natural_conv_factor.unit / target_unit
    ).decompose()
...
def convert(initial_unit, target_unit):
    ...
    if not (
        isinstance(initial_unit, astropy.units.core.UnitBase)
