...
def test_conversions():
    ...
    assert(natpy.convert(1*natpy.cm**(-1)/(natpy.t * natpy.keV), (natpy.t * natpy.yr *
                                                                  natpy.keV)**(-1)) == (9.4607304725808e+17 * (natpy.t * natpy.yr * natpy.keV)**(-1)))

    # switch units, determine Planck length
    natpy.set_active_units("planck")
    # expect: 1 = 1.616255 e-35 m
    assert(natpy.convert(1*natpy.dimensionless_unscaled, natpy.m)
           == 1.6162550244237053e-35 * natpy.m)
    ...
    assert(natpy.convert(1*natpy.GeV**(-3), natpy.fm**3)
           == 0.007683505576381197 * natpy.fm**3)
